...
double CalculatedQuadramp::computeAtTime(double t) {
    ...
    if (t < t3) {
        ...
        double result = calculatedData.acc_distance + calculatedData.ste_speed * abs(calculatedData.ste_time) + calculatedData.ste_speed * local_t -copysign(1, calculatedData.dec_time) * data.acc * pow(local_t,2) / 2.0f;
