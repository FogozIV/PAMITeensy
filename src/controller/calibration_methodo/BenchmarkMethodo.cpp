...
BenchmarkMethodo::BenchmarkMethodo(const std::shared_ptr<BaseRobot> &robot, const std::shared_ptr<Mutex> &sdMutex,
                                   ...
                                   uint8_t benchmark_subtype): CalibrationMethodo(robot, sdMutex),
                                                               benchmark_type(benchmark_type),
                                                               benchmark_subtype(benchmark_subtype) {
...
void PROGMEM BenchmarkMethodo::start() {
    ...
    this->openFile(
        (String("BenchmarkController") + String(rtc_get()) + (benchmark_type == 0
                                                                  ? "ANGLE"
                                                                  : (benchmark_type == 1
                                                                         ? "DISTANCE"
                                                                         : "ANGLE_DISTANCE")) + ".bin").c_str(),
        8192 * 2 * 2);
    buffer->write_raw((uint64_t) BinaryFileType::UNIVERSAL_BENCHMARK_V0_1);
    writeControllerTypeToBuffer(buffer, robot->getController());
    benchmarkComputeHook = robot->addEndComputeHooks([this]() {
        if (robot->isControlDisabled()) return;
        double current_error_distance = multDistance * pow(
                                            (robot->getTranslationalPosition() - robot->getTranslationalTarget()),
                                            2) * robot->getDT();
        double current_error_angle = multAngle * pow(
                                         (robot->getRotationalPosition() - robot->getRotationalTarget()).toDegrees(),
                                         2) * robot->getDT();
        ...
        buffer->write_raw(robot->getPositionTarget().getX());
        buffer->write_raw(robot->getPositionTarget().getY());
        buffer->write_raw(robot->getPositionTarget().getAngle().toDegrees());
        buffer->write_raw(robot->getLeftMotor()->getPWM());
        buffer->write_raw(robot->getRightMotor()->getPWM());
        writeControllerToBuffer(buffer, this->robot->getController(), robot);
    ...
    switch (benchmark_type) {
        case ANGLE:
            switch (benchmark_subtype) {
                case 1: STEP_ANGLE_TARGET(AngleConstants::LEFT);
                    STEP_ANGLE_TARGET(AngleConstants::RIGHT);
                    STEP_ANGLE_TARGET(AngleConstants::ZERO);
                    break;
                default: COMPLETE_ANGLE_TARGET(AngleConstants::LEFT, RampData(90,180));
        ...
        case DISTANCE:
            switch (benchmark_subtype) {
                case 1:
                    STEP_DISTANCE_TARGET(1000);
                    STEP_DISTANCE_TARGET(-2000);
                    STEP_DISTANCE_TARGET(1000);
                    break;
                default: COMPLETE_DISTANCE_TARGET(1000, RampData(200,400));
